...
var widgStylesheet = "editor/css/widgContent.css";
...
widgSelectBlockOptions.push("", "Texto plano");
widgSelectBlockOptions.push("<p>", "Parrafo");
widgSelectBlockOptions.push("<h1>", "Título 1");
widgSelectBlockOptions.push("<h2>", "Título 2");
widgSelectBlockOptions.push("<h3>", "Título 3");
	...
	if (typeof(document.designMode) == "string" && (document.all || document.designMode == "off"))
	{
		...
		for (var i = 0; i < theTextareas.length; i++)
		{
			...
			if (theTextarea.className.classExists("widgEditor"))
			{
				...
				setTimeout(initMe(theTextarea.id), 500 * (i));
...
function initMe(id)
{
	myEditor=new widgEditor(id);
	document.getElementById(id+'WidgToolbarSelectBlock').selectedIndex=0;
}
...
function widgToolbar(theEditor)
{
	...
	for (var i = 0; i < widgToolbarItems.length; i++)
	{
		switch (widgToolbarItems[i])
		{
			...
			case "hyperlink":
				this.addButton(this.theList.id + "ButtonLink", "widgButtonLink", "Enlace", "link");
				
				break;
				
			case "unorderedlist":
				this.addButton(this.theList.id + "ButtonUnordered", "widgButtonUnordered", "Lista de viñetas", "insertunorderedlist");
				
				break;
				
			case "orderedlist":
				this.addButton(this.theList.id + "ButtonOrdered", "widgButtonOrdered", "Lista numerada", "insertorderedlist");
		...
		case "link":
			...
			else
			{
				if (theIframe.contentWindow.document.selection)
				{
					...
					if (theSelection == "")
					{
						alert("Selecciona el texto al que le pondras un enlace");
						break;
					}
				}
				else
...
function widgToolbarCheckState(theWidgEditor, resubmit)
{
	...
	var selectt=document.getElementById(theWidgEditor.myId)
	if (selectt.selectedIndex<0)
	{
